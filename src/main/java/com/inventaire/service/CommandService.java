...
public class CommandService {
    ...
    public Command addCommande(Long id_user ){//int id_user
        Sequence s = new Sequence();
        sequenceService.addSequence(s);
        List<ProduitsCommandes> pc = produitsCommandesService.findAllProduitCommandes(id_user);
        Command command =new Command();
        command.setId_commande(sequenceService.findPlusGrand());
        command.setId_user(id_user);
        float totale =0;
        /* Calcule totale */
        for(int i=0; i<pc.size();i++){
            if(pc.get(i).getId_user()==command.getId_user()){
                ...
                produitsCommandesService.deleteProduitsCommandes(pc.get(i).getId_produit(),id_user);
